...
import com.l2j4team.gameserver.model.zone.type.L2SwampZone;
...
public class PlayerStat extends PlayableStat
{
	...
	/*
	 * public int getPAtkSpd() { if (getActiveChar().isMounted()) { int base = getActiveChar().getPetDataEntry().getMountAtkSpd(); if (getActiveChar().checkFoodState(getActiveChar().getPetTemplate().getHungryLimit())) base /= 2; return (int) calcStat(Stats.POWER_ATTACK_SPEED, base, null, null); }
	 * int val = super.getPAtkSpd(); if (getActiveChar().getClassId().equals(ClassId.PHANTOM_RANGER) || getActiveChar().getClassId().equals(ClassId.GHOST_SENTINEL)) { if ((val > Config.MAX_PATK_SPEED_GHOST)) { return Config.MAX_PATK_SPEED_GHOST; } } else if
	 * (getActiveChar().getClassId().equals(ClassId.SILVER_RANGER) || getActiveChar().getClassId().equals(ClassId.MOONLIGHT_SENTINEL)) { if ((val > Config.MAX_PATK_SPEED_MOONL)) { return Config.MAX_PATK_SPEED_MOONL; } } else { if ((val > Config.MAX_PATK_SPEED)) { return Config.MAX_PATK_SPEED; } }
	 * val += Config.ADD_ATKSPEED; return val; }
	 */
