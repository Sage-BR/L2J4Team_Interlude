...
import com.l2j4team.gameserver.model.actor.ai.Desire;
...
public class CreatureAI extends AbstractAI
{
	...
	{
		if ((target == null) || (getIntention() == CtrlIntention.REST))
		{
	...
	{
		if ((getIntention() == CtrlIntention.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the Player actor
			clientActionFailed();
			return;
	...
	{
		if ((getIntention() == CtrlIntention.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow() || _actor.isMovementDisabled())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the Player actor
			clientActionFailed();
			return;
		...
		if (_actor.isDead() || (_actor == target))
		{
			clientActionFailed();
			return;
		}
	...
	protected void onIntentionPickUp(WorldObject object)
	{
		...
		if ((getIntention() == CtrlIntention.REST) || _actor.isAllSkillsDisabled() || _actor.isCastingNow() || _actor.isAttackingNow())
		{
			clientActionFailed();
			return;
		}
