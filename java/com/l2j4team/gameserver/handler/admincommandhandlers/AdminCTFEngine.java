...
public class AdminCTFEngine implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final Player activeChar)
	{
		...
		switch (comm)
		{
			...
			{
				activeChar.teleToLocation(CTF.get_npcLocation(), 0);
				showMainPage(activeChar);
			}
				...
				if (st.hasMoreTokens())
				{
					for (final String team : CTF._teams)
					{
						if (team.equals(st.nextToken()))
						{
							...
							activeChar.teleToLocation(CTF._teamsX.get(index), CTF._teamsY.get(index), CTF._teamsZ.get(index), 0);
							return true;
						}
					}
			...
			case admin_ctf_tele_flag:
				...
				if (st.hasMoreTokens())
				{
					...
					{						
						final int index = CTF._teams.indexOf(team);
						activeChar.teleToLocation(CTF._flagsX.get(index), CTF._flagsY.get(index), CTF._flagsZ.get(index), 0);						
					}
	...
	public void showEditPage(final Player activeChar)
	{
	...
	public void showControlPage(final Player activeChar)
	{
		final NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		final StringBuilder replyMSG = new StringBuilder("<html><body>");
