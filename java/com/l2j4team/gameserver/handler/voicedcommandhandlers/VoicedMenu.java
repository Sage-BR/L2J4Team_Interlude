...
import com.l2j4team.events.PartyZoneTask;
...
import com.l2j4team.gameserver.ArenaEvent;
import com.l2j4team.gameserver.AutofarmManager;
import com.l2j4team.gameserver.MissionReset;
import com.l2j4team.gameserver.NewZoneVote;
import com.l2j4team.gameserver.PartyFarmEvent;
import com.l2j4team.gameserver.Restart;
...
{
	public static final Logger _log = Logger.getLogger(VoicedMenu.class.getName());
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		else if (command.startsWith("radius"))
		{
			...
			if (params.length > 1)
			{
				...
				Config.AUTO_FARM_RADIUS = radius;
				updateAutofarmRadius(activeChar.getObjectId(), radius);
				sendVipWindow(activeChar);
				activeChar.sendMessage("AutoFarm Range Change For:" + radius);
				activeChar.sendPacket(new ExShowScreenMessage("AutoFarm Range Change For:" + radius, 2000));
				
			}
		}
		
		else if (command.startsWith("respecthunt"))
		{
			if (activeChar.isInRespectHunt())
			{
				activeChar.setRespectHunt(false);
				updateRespectHuntStatus(activeChar.getObjectId(), false);
				sendVipWindow(activeChar);
				activeChar.sendMessage("Respect Hunt deactivated.");
				activeChar.sendPacket(new ExShowScreenMessage("Respect Hunt Off.", 2000));
			}
			else
			{
				activeChar.setRespectHunt(true);
				updateRespectHuntStatus(activeChar.getObjectId(), true);
				sendVipWindow(activeChar);
				activeChar.sendMessage("Respect Hunt activated.");
				activeChar.sendPacket(new ExShowScreenMessage("Respect Hunt On.", 2000));
			}
		}
		
		else if (command.startsWith("skillbar"))
		{
			String[] params = command.split(" ");
			if (params.length > 1)
			{
				String skillbarStr = params[1];
				int skillbar = -1;
				...
				if (skillbar >= 0 && skillbar <= 9)
				{
					Config.AUTO_FARM_BAR = skillbar;
					updateAutofarmSkillbar(activeChar.getObjectId(), skillbar);
					sendVipWindow(activeChar);
					String skillbarMessage = "AutoFarm Skill Bar Change For: " + (skillbar == 0 ? "F1" : "F" + (skillbar + 1));
					activeChar.sendMessage(skillbarMessage);
					activeChar.sendPacket(new ExShowScreenMessage(skillbarMessage, 2000));
				}
			}
		}
	...
	private static void updateAutofarmRadius(int charId, int radius)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("UPDATE Characters_Autofarm SET radiusfarm=? WHERE obj_id=?"))
		{
		...
		{
			_log.log(Level.WARNING, "Failed to update Autofarm radius for character " + charId, e);
		}
	}
	
	private static void updateRespectHuntStatus(int charId, boolean respectHuntStatus)
	{
		String status = respectHuntStatus ? "On" : "Off";
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("UPDATE Characters_Autofarm SET respecthunt=? WHERE obj_id=?"))
		{
		...
		{
			_log.log(Level.WARNING, "Failed to update Respect Hunt status for character " + charId, e);
		}
	}
	
	private static void updateAutofarmSkillbar(int charId, int skillbar)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("UPDATE Characters_Autofarm SET skillbar=? WHERE obj_id=?"))
		{
			statement.setInt(1, skillbar);
			statement.setInt(2, charId);
			statement.executeUpdate();
		}
		catch (SQLException e)
		{
			_log.log(Level.WARNING, "Failed to update Autofarm skillbar for character " + charId, e);
		}
	}
