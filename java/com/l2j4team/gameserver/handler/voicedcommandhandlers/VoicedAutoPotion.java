...
public class VoicedAutoPotion implements IVoicedCommandHandler {
    ...
    public boolean useVoicedCommand(String command, Player activeChar, String params) {
        ...
        if (command.equals("acpon"))
        {
			...
			{
				activeChar.sendMessage("nao e permitido usar Auto Potion Com arma Do CTF.");
				return false ;
			}

			if(activeChar.isDead())
			{
				activeChar.sendMessage("nao e permitido usar Auto Potion morto.");
				return false ;
			}
			if(activeChar.isInOlympiadMode())
			{
				activeChar.sendMessage("nao e permitido usar Auto Potion nas Olympiadas.");
				return false ;
			}
...
//                               Checking our health
                                if ((activeChar.getStatus().getCurrentHp() / activeChar.getMaxHp()) * 100 < ACP_HP_LVL && ACP_HP)
                                {
                                   ...
                                   if(activeChar.isDead())
                                   {
                               	   activeChar.sendMessage("nao e permitido Auto Potion morto.");
                                	   return ;
                                   }
                                   if(activeChar._inEventCTF && activeChar._haveFlagCTF)
                   					{
                                   ...
                                   if(activeChar.isInOlympiadMode())
                                   {
                                	   activeChar.sendMessage("nao e permitido Auto Potion Olympiadas.");
                                	   return ;
                                   }
                                    if (handlerHP != null)
                                    {
