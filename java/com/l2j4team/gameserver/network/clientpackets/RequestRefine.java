...
import com.l2j4team.gameserver.network.serverpackets.ExVariationResult;
...
public final class RequestRefine extends AbstractRefinePacket
{
	...
	protected void runImpl()
	{
		...
		if ((gemStoneItem == null) || !isValid(activeChar, targetItem, refinerItem, gemStoneItem))
		{
			activeChar.sendPacket(new ExVariationResult(0, 0, 0));
			activeChar.sendPacket(SystemMessageId.AUGMENTATION_FAILED_DUE_TO_INAPPROPRIATE_CONDITIONS);
			return;
		...
		// Consume gemstones
		if (!activeChar.destroyItem("RequestRefine", refinerItem, 1, null, false) || !activeChar.destroyItem("RequestRefine", gemStoneItem, _gemStoneCount, null, false))
		{
			activeChar.sendPacket(new ExVariationResult(0, 0, 0));
			activeChar.sendPacket(SystemMessageId.AUGMENTATION_FAILED_DUE_TO_INAPPROPRIATE_CONDITIONS);
			return;
