...
import com.l2j4team.gameserver.AutofarmConstants;
import com.l2j4team.gameserver.communitybbs.Manager.MailBBSManager;
...
import com.l2j4team.gameserver.handler.voicedcommandhandlers.VoicedDonate;
...
import com.l2j4team.gameserver.instancemanager.NewsHtml;
...
import com.l2j4team.gameserver.instancemanager.VIPFreeHTML;
//import com.l2j4team.gameserver.instancemanager.VIPFreeHTML;
...
import com.l2j4team.gameserver.model.entity.ClanHall;
...
import com.l2j4team.gameserver.network.serverpackets.Die;
import com.l2j4team.gameserver.network.serverpackets.EtcStatusUpdate;
import com.l2j4team.gameserver.network.serverpackets.ExMailArrived;
...
import com.l2j4team.gameserver.network.serverpackets.PledgeSkillList;
import com.l2j4team.gameserver.network.serverpackets.PledgeStatusChanged;
import com.l2j4team.gameserver.network.serverpackets.QuestList;
...
import com.l2j4team.gameserver.util.HWID;
import com.l2j4team.gameserver.util.IPLog;
...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.getFirstLog())
		{
			...
			{
				String respectHuntStatus = activeChar.isInRespectHunt() ? "On" : "Off";
				saveAutofarmData(activeChar.getObjectId(), activeChar.getName(), AutofarmConstants.targetingRadius, AutofarmConstants.shortcutsPageIndex, respectHuntStatus);
			}
		}
		
		if (Config.ENABLE_AUTO_FARM)
		{
			if (isRespectHuntEnabled(activeChar.getObjectId()))
			{
	...
	private static void saveAutofarmData(int charId, String charName, int radiusFarm, int skillBar, String respectHunt)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("REPLACE INTO Characters_Autofarm (obj_id, char_name, radiusfarm, skillbar, respecthunt) VALUES (?, ?, ?, ?, ?)"))
		{
			statement.setInt(1, charId);
			statement.setString(2, charName);
			statement.setInt(3, radiusFarm);
			statement.setInt(4, skillBar);
			statement.setString(5, respectHunt);
			statement.executeUpdate();
		}
		catch (SQLException e)
		{
			_log.log(Level.WARNING, "Failed to save Autofarm data for character " + charId, e);
		}
	}
