...
import com.l2j4team.gameserver.model.multisell.Entry;
import com.l2j4team.gameserver.model.multisell.Ingredient;
...
public class MultiSellChoose extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		if (!FloodProtectors.performAction(getClient(), Action.MULTISELL) || _amount < 1 || _amount > 9999)
		{
		...
		if ((npc != null && !list.isNpcAllowed(npc.getNpcId())) || (npc == null && list.isNpcOnly()) && !player.isCMultisell())
		{
			player.setMultiSell(null);
			return;
		}
		
		if (npc != null && !npc.canInteract(player) && !player.isCMultisell())
		{
			player.setMultiSell(null);
			return;
		}
		...
		for (Entry entry : list.getEntries())
		{
			if (entry.getId() == _entryId)
			{
				...
				if (!inv.validateCapacity(slots) || weight > Integer.MAX_VALUE || weight < 0 || !inv.validateWeight((int) weight))
				{
				...
				for (Ingredient e : ingredientsList)
				{
					...
					else if (e.getItemId() == 65436 && e.getItemCount() * _amount > player.getPcBangScore())
					{
						player.sendPacket(new SystemMessage(SystemMessageId.NOT_ENOUGH_ITEMS));
						return;
					}
					else if (e.getItemId() != 65336 && e.getItemId() != 65436)
					{
