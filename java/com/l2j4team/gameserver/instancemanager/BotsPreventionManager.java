...
import com.l2j4team.gameserver.network.serverpackets.PledgeCrest;
...
public class BotsPreventionManager
{
	...
	static void validationwindow(Player player)
	{
		...
		{
			StringUtil.append(tb, "<td><button action=\"link report_" + i + "\" width=32 height=32 back=\"Crest.crest_" + Config.SERVER_ID + "_" + (container.options.get(i) + 1500) + "\" fore=\"Crest.crest_" + Config.SERVER_ID + "_" + (container.options.get(i) + 1500) + "\"></td>");
		}
	...
	public void Classes(String command, final Player activeChar)
	{
		...
		{
			BotsPreventionManager.getInstance().AnalyseBypass(command, activeChar);
	...
	public void AnalyseBypass(String command, Player player)
	{
		...
		if (choosenoption > -1)
		{
			...
			else
			{
				...
				player.setCaptcha_ok(player.getCaptcha_ok() + 1);
