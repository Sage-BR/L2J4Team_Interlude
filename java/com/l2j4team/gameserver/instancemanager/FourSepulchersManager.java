...
import com.l2j4team.gameserver.model.actor.instance.SepulcherMonster;
import com.l2j4team.gameserver.model.actor.instance.SepulcherNpc;
...
public class FourSepulchersManager
{
	...
	public synchronized void tryEntry(Npc npc, Player player)
	{
		...
		if (party == null || party.getMemberCount() < Config.FS_PARTY_MEMBER_COUNT)
		{
		...
		for (Player member : party.getPartyMembers().stream().filter(m -> !m.isDead() && MathUtil.checkIfInRange(700, player, m, true)).collect(Collectors.toList()))
		{
	...
	public void managerSay(byte min)
	{
		...
		else if (_inEntryTime)
		{
			...
			for (L2Spawn temp : _managers)
			{
				if ((temp == null) || !(temp.getNpc() instanceof SepulcherNpc))
				{
