...
public class PartyZoneManager
{
	...
	private static boolean checkClanAreaKickTask(Player activeChar, Integer numberBox)
	{
		...
		if (clan != null)
		{
			...
			{
				if (!player.isInsideZone(ZoneId.FLAG) || player.getClan() == null)
					continue;
	...
	private static boolean checkPlayersKickTask(Player activeChar, Integer numberBox)
	{
		...
		if (activeChar != null)
		{
			...
			{
				if (!player.isInsideZone(ZoneId.FLAG) || player.getHWID() == null)
					continue;
	...
	private static boolean checkAllyAreaKickTask(Player activeChar, Integer numberBox, Collection<Player> world)
	{
		...
		if (activeChar.getAllyId() != 0)
		{
			...
			{
				if (!player.isInsideZone(ZoneId.FLAG) || player.getAllyId() == 0)
					continue;
	...
	public boolean checkPlayersArea(Player activeChar, Integer numberBox, Boolean forcedTeleport)
	{
		if (checkPlayersKickTask(activeChar, numberBox))
		{
			if (forcedTeleport)
			{
				activeChar.sendPacket(new CreatureSay(0, Say2.TELL, "SYS", "Allowed only " + numberBox + " Client in Party Zone!!"));
				for (Player allgms : World.getAllGMs())
				{
					if (!activeChar.isGM())
						allgms.sendPacket(new CreatureSay(0, Say2.TELL, "[Double HWID]", activeChar.getName() + " in Party Zone!"));
				}
				RandomTeleport(activeChar);
			}
			return true;
	...
	private static boolean checkPlayersKickTask_ip(Player activeChar, Integer numberBox, Collection<Player> world)
	{
		...
		for (Player player : world)
		{
			if (!player.isInsideZone(ZoneId.FLAG) || player.getClient() == null || player.getClient().isDetached())
				continue;
	...
	public boolean checkPlayersArea_ip(Player activeChar, Integer numberBox, Collection<Player> world, Boolean forcedLogOut)
	{
		if (checkPlayersKickTask_ip(activeChar, numberBox, world))
		{
			if (forcedLogOut)
			{
				activeChar.sendPacket(new CreatureSay(0, Say2.TELL, "SYS", " Double box not allowed in Party Zone!"));
				for (Player allgms : World.getAllGMs())
				{
					if (!activeChar.isGM())
						allgms.sendPacket(new CreatureSay(0, Say2.TELL, "[Double IP]", activeChar.getName() + " in Party Zone!"));
				}
				RandomTeleport(activeChar);
			}
			return true;
	...
	public static synchronized boolean IPProtection(final String ip)
	{
		...
		{
			final PreparedStatement statement = con.prepareStatement("SELECT ip FROM ip_protect_partyzone WHERE ip=?");
